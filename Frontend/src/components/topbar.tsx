...
const topbar = () => {
  ...
  return (
    <>
    <div className="header sticky top-0 left-0 w-full bg-[url('/bgg.jpg')] bg-cover bg-right shadow-md px-10">
        ...
        <div className="bottom flex justify-between items-center py-2 bx-2">
          <div className="media  flex justify-center gap-8 py-2">
            {mediaTypes.map((type) => (
            <button className='text-2xl bg-gray-600 text-[#eee] px-2 py-1 rounded cursor-pointer hover:bg-gray-900' key={type}>{type}</button>
          ))}
          </div>
